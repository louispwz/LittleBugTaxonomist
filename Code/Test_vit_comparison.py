...
def main(tar_path="Data/small_database.tar", metadata_path="Data/metadata_images.json"):
    ...
    model.to(device)
    # build de candidate_info et mapping metadata (nécessaire pour construire les prompts hiérarchiques)
    candidate_info = {}
    for bug in meta:
        gb = bug.get('gbif') or {}
    ...
    # texte constant
    # prompts = [f"image of an insect" for _ in unique_candidate]
    # texte random
    # prompts = ["".join(random.choices(string.ascii_uppercase + string.digits, k=10)) for _ in unique_candidate]
    ...
    out_path = os.path.join(os.path.dirname(metadata_path) or '.', 'viT_predictions_hierarchical.json')
